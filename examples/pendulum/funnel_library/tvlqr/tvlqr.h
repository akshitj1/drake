...
// todo: change return type to TimeVaryingAffineSystem as
// LinearQuadraticRegulator returns AffineSystem
class TimeVaryingLQR final : public LeafSystem<double> {
 ...
 private:
  ...
  // todo: replace by input ports for state and input trajectory ref:
  // PidController
 ...
 public:
  TimeVaryingLQR(const System<double>& _system, const PPoly<double>& _x_des,
                 const PPoly<double>& _u_des, const MatrixX<double>& _Q,
                 const MatrixX<double>& _R, const MatrixX<double>& _Qf)
      : system(_system),
        Q(_Q),
        Qf(_Qf),
        R(_R),
    ...
    output_index_control_ =
        this->DeclareVectorOutputPort(
                "corrected_control", BasicVector<double>(kNumInputs),
                &TimeVaryingLQR::CalcControl, {this->all_state_ticket()})
            .get_index();

    input_index_state_ =
        this->DeclareInputPort(kVectorValued, kNumStates).get_index();

    computeSTrajectory();
  }

  /*                template <typename U>
                                                                  TimeVaryingLQR(const
     TimeVaryingLQR<U>& other) : TimeVaryingLQR(other.system, other.x_des,
     other.u_des, other.Q, other.R, other.Qf) {}*/
  ...
  /*
                                                                  // todo:
     implement this void DoCalcTimeDerivatives(const systems::Context<double>&
     context, systems::ContinuousState<double> *derivatives) const override {}
  */

 private:
  double revTime(const double& t) const { return revTime(t, kTimeSpan); }
 ...
 public:
  void CalcControl(const Context<double>& context,
    ...
    const Eigen::VectorBlock<const VectorX<double>> x =
    ...
    auto affine_system =
        Linearize<double>(system, x_des.value(t), u_des.value(t));

    auto K = R.inverse() * affine_system->B().transpose() * S;
    VectorX<double> u_delta = -K * x_bar;
    drake::log()->debug("time : {}\tu_des : {}\tu_delta : {}", t,
                        u_des.value(t), u_delta);
 ...
 private:
  void computeSTrajectory() {
    ...
    InitialValueProblem<double> ivp(sDot<double>(system, Q, R, x_des, u_des),
                                    kDefaultValues);
 ...
 public:
  /**
   * Computed S trajectory is in DenseOutput form which does not provides
   * derivatives hence convert to PPoly
   */
  void getSKTrajectory(PPoly<double>& S_t, PPoly<double>& K_t) {
    const std::vector<double> breaks(u_des.get_segment_times());
    std::vector<MatrixX<double>> S_knots(breaks.size());
    std::vector<MatrixX<double>> K_knots(breaks.size());

    for (int k = 0; k < int(breaks.size()); k++) {
      auto t = breaks[k];
      const double rev_t = this->revTime(t);
      auto s = s_traj->Evaluate(rev_t);
      auto S = unravel<double>(s, kNumStates);
      S_knots[k] = S;
      auto affine_system =
          Linearize<double>(system, x_des.value(t), u_des.value(t));
      const MatrixX<double> B = affine_system->B();
      K_knots[k] = R.inverse() * B.transpose() * S;
    }
    log()->info("breaks: {}, knots: {}\n", breaks.size(), S_knots.size());
    S_t = PPoly<double>::Cubic(breaks, S_knots, false);
    K_t = PPoly<double>::Cubic(breaks, K_knots, false);
  }

 private:
  // todo: Should we use lambda instead? trade-off between readability
  // and compactness
  template <typename T>
  class sDot {
   private:
   ...
   public:
    sDot(const System<T>& system, const MatrixX<T>& Q, const MatrixX<T>& R,
         const PPoly<T>& x_des, const PPoly<T>& u_des)
        : system(system),
          Q(Q),
          R(R),
          kNumStates(Q.cols()),
    ...
    VectorX<T> operator()(const T& t, const VectorX<T>& _s,
                          const VectorX<T>& params) const {
      unused(params);
      ...
      // we have to reverse the desired trajectory to use ivp by
      // converting backward integration to forward in time todo:
      // ideally reverse the trajectory itself, as top level handling
      // is more graceful
      const double rev_t = TimeVaryingLQR::revTime(t, kTimeSpan);

      auto affine_system =
          Linearize<T>(system, x_des.value(rev_t), u_des.value(rev_t));

      const MatrixX<T> A = affine_system->A();
      const MatrixX<T> B = affine_system->B();
      const MatrixX<T> Ri = R.inverse();

      // sign is changed as we are converting backward to forward
      // integration
   ...
   /*        namespace scalar_conversion {
                                                   template <>
                                                   struct
      Traits<controllers::TimeVaryingLQR> : public NonSymbolicTraits {};
                                   }*/
}  // namespace systems
}  // namespace drake
