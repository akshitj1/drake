...
*/
						...
						public:
								void CalcControl(const Context<double> &context,
										...
										drake::log()->debug("time : {}\tu_des : {}\tu_delta : {}",
																				t, u_des.value(t), u_delta);
						...
						void getSKTrajectory(PPoly<double> &S_t, PPoly<double>& K_t){
							...
							std::vector<MatrixX<double>> K_knots(
									breaks.size());

							for(int k=0;k<int(breaks.size()); k++){
								auto t =  breaks[k];
								auto s = s_traj->Evaluate(t);
								auto S = unravel<double>(s, kNumStates);
								S_knots[k] = S;
								auto affine_system = Linearize<double>(system, x_des.value(t),
																									u_des.value(t));
								const MatrixX<double> B = affine_system->B();
								K_knots[k] = -R.inverse() * B.transpose() * S;
							}
							std::cerr<<"breaks: "<<breaks.size()<<", knots: "<<S_knots.size()<<std::endl;
					 ...
					 private:
						...
						class sDot {
						 ...
						 public:
							sDot(const System<T>& system, const MatrixX<T>& Q,
									 const MatrixX<T>& R, const PPoly<T>& x_des,
									 const PPoly<T>& u_des)
									: system(system),
										Q(Q),
										R(R),
							...
							VectorX<T> operator()(const T& t, const VectorX<T>& _s,
								...
								MatrixX<T> S(kNumStates, kNumStates),
										Sdot(kNumStates, kNumStates);
								...
								auto affine_system = Linearize<T>(system, x_des.value(rev_t),
																									u_des.value(rev_t));
								...
								// integration
								Sdot = S * A + A.transpose() * S -
											 S * B * Ri * B.transpose() * S + Q;
