...
class TailsitterPlantBuilder {
 ...
 public:
  ...
  MultibodyPlant<double>* build(systems::DiagramBuilder<double>& builder,
    ...
    // multibody::RevoluteSpring<double>* left_joint_spring;
    // multibody::RevoluteSpring<double>* right_joint_spring;
    ...
    for (int i = 0; i < 2; i++) {
      ...
      const auto joint = &plant->AddJoint<RevoluteJoint>(
          ...
          std::optional<RigidTransform<double>>{}, Vector3<double>::UnitX(), 0.1);

      plant->AddJointActuator(std::string("tau_") + (i == 0 ? "left" : "right"), *joint);

      plant->AddForceElement<multibody::RevoluteSpring>(*joint, 0., 10.);
